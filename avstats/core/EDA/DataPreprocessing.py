...
class DataPreprocessing:
    ...
    def check_missing_and_duplicates(self) -> dict:
        ...
        duplicate_rows = self.df[self.df.duplicated(subset=[self.unique_column], keep=False)].copy()

        # Ensure duplicate_rows is a valid DataFrame
        if duplicate_rows.empty:
            duplicate_rows = pd.DataFrame(columns=self.df.columns)  # Return an empty DataFrame with correct columns
    ...
    def check_time_consistency(self) -> tuple[DataFrame, DataFrame]:
        ...
        inconsistent_rows = self.df[self.df['inconsistency_flag'] == 1].copy()
        inconsistent_flights_count = len(inconsistent_rows)
        inconsistent_flights_percentage = (inconsistent_flights_count / total_flights) * 100

        # Keep only consistent rows
        self.df = self.df[self.df['inconsistency_flag'] == 0]
        ...
        for feature, outlier_df in outliers.items():
            if method == "remove":
                ...
                if detection_method == "IQR":
                    ...
                    iqr = q3 - q1
                    lower_bound = q1 - threshold * iqr
                    upper_bound = np.nextafter(q3 + threshold * iqr, -np.inf)  # Smallest number before upper_bound
